...
SemiconductorsWindow::SemiconductorsWindow(QWidget *parent) : QWidget(parent)
{
    ...
    setWindowTitle(codec->toUnicode("Поиск всех полупроводников"));
    ...
    discLabel -> setText(codec->toUnicode("Найденные полупроводники: "));
...
void SemiconductorsWindow::AddTable(std::vector<Element> origin,int size)
{
    table->setHorizontalHeaderLabels(list);
    int i =0;
...
void SemiconductorsWindow::FillTextPP()
{
    table ->setRowCount(0);
    std::string line;
    int counter = 0;
    ...
    if(true)
    {
        ...
        while(std::getline(note,line,'\n'))
        {
            ...
            std::getline(in,mass,'|');
            std::getline(in,type,'|');
            if (type.find("олупроводник")!= std::string::npos)
            {
...
void SemiconductorsWindow::exit()
{
    QWidget::close();
}
